...
import { api, type VideoJobDetail } from "~/lib/api";
...
export default function VideoRun() {
  ...
  async function loadEvents() {
    ...
    if (data) { data.forEach((e: any) => seen.current.add(e.seq)); setEvents(data as Ev[]); }
  }
  ...
  // Populate the editable plan when the run pauses for review.
  useEffect(() => {
  ...
  if (!job) return <main style={main}><p style={{ color: "#666" }}>{err ?? "Loading…"}</p></main>;

  const activeIdx = STAGE_INDEX[job.status] ?? 0;
  const failed = job.status === "failed" || job.status === "cancelled";

  return (
    <main style={main}>
      <p style={{ marginBottom: 4 }}><Link to="/app/video" style={{ color: ACCENT }}>← Videos</Link></p>
...
const main: React.CSSProperties = { maxWidth: 820, margin: "0 auto", padding: "2rem 1.25rem" };
